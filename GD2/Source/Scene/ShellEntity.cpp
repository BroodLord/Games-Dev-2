	...
	bool CShellEntity::Update(TFloat32 updateTime)
	{
		...
		if (Timer >= 0)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				CEntity* TankObject = EntityManager.GetEntity(TankUID);

				if (Matrix().Position().x > TankObject->Matrix().Position().x - 2 && Matrix().Position().x < TankObject->Matrix().Position().x + 2 &&
					Matrix().Position().y > TankObject->Matrix().Position().y - 2 && Matrix().Position().y < TankObject->Matrix().Position().y + 2 &&
					Matrix().Position().z > TankObject->Matrix().Position().z - 2 && Matrix().Position().z < TankObject->Matrix().Position().z + 2)
				{
					if (GetName() != TankObject->GetName())
					{
						Messenger.SendMessageA(TankUID, msg);
						Timer = 2.0f;
						return false;
					}
				}
		...
		Matrix().MoveLocalZ(80 * updateTime);
